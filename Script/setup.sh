#!/bin/bash
echo Please make sure you have ssh public key "use ssh-keygen command"
...
touch sendVector.sh
echo "ssh root@raspberrypi3b.local python2.7 piCamera.py $name@$host $tmp_path/tmp" > sendVector.sh
chmod +x sendVector.sh
